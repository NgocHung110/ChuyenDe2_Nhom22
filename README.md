```markdown
🏥 Phân Vùng Da Ung Thư Sử Dụng Mô Hình UNet

📋 Giới Thiệu Dự Án

Dự án này phát triển hệ thống tự động phân vùng vùng da bị ung thư bằng mô hình học sâu UNet. Hệ thống giúp hỗ trợ bác sĩ trong việc chẩn đoán sớm và chính xác các tổn thương da.

✨ Tính Năng Chính

- 🎯 **Phân vùng chính xác** vùng da ung thư từ ảnh đầu vào
- 🤖 **Mô hình UNet** với skip connections cho kết quả tốt nhất
- 📊 **Đánh giá đa chiều** với các metrics: IoU, Dice, Accuracy
- 🚀 **Tốc độ cao** với inference time ~35ms/ảnh


📈 Kết Quả Đạt Được

| Metric | Train | Validation |
|--------|-------|------------|
| IoU | 0.8472 | 0.8416 |
| Dice Coefficient | 0.9169 | 0.9138 |
| Pixel Accuracy | 0.9738 | 0.9597 |

![Kết quả phân vùng](results/prediction_examples.png)

👥 Thành Viên Nhóm

| STT |     Họ Tên      |    MSSV   |
|-----|-----------------|-----------|
| 1   |   Tô Ngọc Hoan  | 106210213 | 
| 2   | Nguyễn Ngọc Hưng| 106210216 | 

## 📞 Liên Hệ

Nếu có thắc mắc về dự án, vui lòng liên hệ:

- Email: nguyenngochung110@gmail.com

## 📄 Giấy Phép

Dự án được phát triển cho mục đích học thuật. Mọi người có thể sử dụng với điều kiện ghi rõ nguồn.

---
